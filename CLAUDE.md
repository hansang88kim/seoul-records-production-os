...
> 작업 시작 전 오리엔테이션 순서: **① §0 작업 철학 → ② `ARCHITECTURE.md`(에이전트용 좌표·
> mermaid 지도) + §2 아키텍처 → ③ `AGENTS.md`(실전 함정/노하우) → ④ §5 "자주 건드리는 파일" 표.**
...
providers/      ── 외부 연동 레이어 (교체 가능한 provider 패턴, mock 항상 존재)
  providers/ai/base.py       작사/스타일 LLM (OpenAI/Gemini/Mock) + SONG_MOODS + 시스템프롬프트
  providers/ai/languages.py  언어별(한/일/태/베/인니/영/스페인) 가사·도시 설정
...
| 썸네일 텍스트 오버레이/폼 | `services/thumbnail/html_renderer.py`, `services/thumbnail/form_prompt_builder.py` |
