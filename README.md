...
**AI Music Label Production Harness — v1.0.0-alpha.42**
...
**v1.0.0-alpha.42: Locked-In Visualizer Defaults + Typeable Fields + Calmer Motion — three requests after further visual testing. (1) The settings the user settled on became the new `visualizer_config()` defaults: height=60 (was 160), opacity=0.20 (was 0.85/0.55 — two inconsistent old defaults), bottom_margin=0 (was 40), width_percent=30 (was 25, itself changed from 100 in alpha.41), glow_strength=0.0 (was 3.0 — unused by classic_bars anyway). Color-theme selector now defaults to '매혹 마젠타' (#ff4d6d) instead of '직접 선택'. (2) Dragging a mouse across a wide slider range is fiddly, so every position/size control (height, width, bottom margin, opacity, the new motion-smoothing control) now has a typeable number field next to it. New `_linked_slider()` helper in app/tabs/video_renderer.py: a slider + `st.number_input()` pair kept in sync via `on_change` callbacks — whichever was touched last wins and updates the other. (3) 'Calmer' motion: added FFmpeg showfreqs' own `averaging` option (0 = only the current frame's peaks — jitteriest; higher = smooths in historic frames — calmer/gentler). Default raised from FFmpeg's own default of 1 to 6, exposed as a new '모션 부드러움 (평활화)' slider+field (0-20). `visualizer_config()` gained the `averaging` parameter (never negative); both `build_visualizer_filter()` (deprecated standalone helper) and the real renderer (`filter_complex_builder.add_visualizer_layer()`) now include `averaging={N}` in the generated `showfreqs` filter string. The now-unused '글로우 강도' slider was removed from the UI (glow_strength stays in the config shape for stability, just fixed at 0.0 — classic_bars never read it anyway). 8 test changes/additions: updated the alpha.41 width-percent-25 test to 30, added a combined defaults-match-user-settled-values test, and averaging default/bounds/filter-string tests for both the standalone helper and the real renderer. 759 tests passing; verified with a headless Streamlit smoke boot and a direct Streamlit AppTest run of the Video Renderer page (confirms the new linked slider+number_input widgets render without exception).**

**v1.0.0-alpha.41: Fix the OTHER Half of the Left-Heavy Bars (ascale=log) + Default 25% Width — a real preview render after alpha.40 still showed bars bunched left. fscale=log alone only fixes WHICH x-position a frequency maps to; it does nothing for bar HEIGHT. ascale was still linear, and bass genuinely has far higher raw FFT magnitude than treble in real music — so even with frequency bins correctly spread out, the loud bass bars dwarfed the quieter (but present) treble bars, and the right side still looked empty. Switched ascale=lin to ascale=log as well: log amplitude compresses the dynamic range so quiet high-frequency content becomes visually comparable to the bass instead of vanishing next to it. ascale=log + fscale=log together are what actually gives the classic, evenly-populated hi-fi-equalizer look. Also changed the default width_percent from 100 to 25 (and the UI slider's default to match) — the narrower band the user settled on after testing. Fixed in both the deprecated standalone helper (build_visualizer_filter) and the real renderer (filter_complex_builder.add_visualizer_layer). 3 test changes: updated the alpha.40 assertion that expected ascale=lin, added a direct regression test asserting ascale=lin is no longer present, and a test for the new default width_percent=25. 755 tests passing; verified with a headless Streamlit smoke boot.**

**v1.0.0-alpha.40: Classic Bars Only + Fixed Log Frequency Scale — per user direction, cut the visualizer down from 9 styles to just **classic_bars** (minimal_dots/minimal_wave/soft_eq_bars/citypop_glow/mirrored_bars/lissajous_scope/spectrum_fire/spectrum_terrain all removed). Also fixed a real visual bug the user spotted in a real render: bars were bunched almost entirely on the left with the high-frequency end barely moving. The bug wasn't the AMPLITUDE scale (already `ascale=lin`) — it was the FREQUENCY axis: FFmpeg's `showfreqs` defaults to a linear frequency scale, so most of a typical song's energy (bass/low-mid) crams into the left ~20% of the bar and treble sits nearly flat. Real equalizers display frequency on a LOG scale for exactly this reason. Added `fscale=log` to the `showfreqs` filter (both the deprecated standalone helper and the real renderer in filter_complex_builder.add_visualizer_layer), which spreads bass out and gives treble a comparable share of screen space — the classic evenly-populated EQ-bar look. `services/video/visualizer.py`: `VISUALIZER_STYLES = ["classic_bars"]`; `visualizer_config()` now defaults to `classic_bars` (was `citypop_glow`) and falls back to it for any unrecognized style value. `app/tabs/video_renderer.py`: the style dropdown is gone (nothing left to choose) — a disabled text field shows "Classic Bars"; the color-theme picker and free-form color picker are unchanged (still useful since classic_bars is fully color-configurable, unlike the removed fixed-palette spectrum styles). Removed the now-invalid alpha.37 tests for the deleted styles; added focused tests for the single remaining style, the log-frequency-scale fix (both the standalone helper and the real renderer), the new default, and the unknown-style fallback. 753 tests passing; verified with a headless Streamlit smoke boot.**
...
│   └── tabs/                       # 실제 라이브 탭 (main.py 내비 순서)
│       ├── song_lab.py             # 곡 생성 (Suno) — Quick/Auto Batch/프로젝트
│       ├── thumbnail_studio.py     # 썸네일 16:9 + 커버 1:1
│       ├── video_renderer.py       # MP3→플레이리스트 영상(ffmpeg)
│       ├── youtube_package.py      # YouTube 메타데이터/업로드 패키지
│       ├── production_qa_tab.py    # Production QA
│       ├── unitedmasters_tab.py    # UnitedMasters 유통
│       ├── project_library.py, project_screen.py  # 프로젝트/라이브러리
│
├── agents/
