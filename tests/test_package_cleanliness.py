"""
Tests for package cleanliness and structural correctness.
...
Windows compatibility:
  ...
  - Path comparisons use .as_posix() for cross-platform safety
"""
import pytest
...
def test_provider_registry_is_only_in_init():
    ...
    for fname in [
        ...
        src = (ROOT / "providers" / "suno" / fname).read_text(encoding="utf-8")
        assert "def get_composer_provider" not in src, (
            f"get_composer_provider must not be defined in {fname}"
        )
    init_src = (ROOT / "providers" / "suno" / "__init__.py").read_text(encoding="utf-8")
...
def test_generate_album_imports_from_registry():
    ...
    src = (ROOT / "workflows" / "generate_album.py").read_text(encoding="utf-8")
...
def test_no_deprecated_utcnow_in_source():
    ...
    Windows compat: uses .as_posix() for path comparison.
    """
...
def test_mock_provider_has_fast_mode():
    ...
    src = (ROOT / "providers" / "suno" / "mock_suno.py").read_text(encoding="utf-8")
