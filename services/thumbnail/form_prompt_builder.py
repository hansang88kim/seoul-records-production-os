# -*- coding: utf-8 -*-
"""
services/thumbnail/form_prompt_builder.py — 썸네일 형태별 이미지 생성 프롬프트 빌더
(v1.0.0-alpha.70). Gemini(Nano Banana) / GPT-Image 등 text-to-image 모델용.

claude.ai 세션에서 확정한 6형태(JazzNe 벤치마킹) 디자인에 맞춰, 형태(A~F)별
구도 제약을 이미지 생성 프롬프트에 강제 주입한다 — 이렇게 해야
services/thumbnail/html_renderer.py가 텍스트를 올릴 여백이 실제로 비어 있다.
prompt_generator.py의 generate_flow_prompt(form=...)가 build_image_prompt()의
composition 문자열을 기존 country/theme 프롬프트에 합성하는 방식으로 통합됨
(이 모듈 자체는 country_presets.py와 독립적 — 통합 지점은 prompt_generator.py).
